...
struct
  ...
  struct
    ...
    let bits_of_val = function
      ...
      | Str _ -> raise(Invalid_argument "bits_of_val")
      | Lval v -> bits_of_width(typeof_var v)
      | Name _ -> bits_of_width( Vine.addr_t)

    let rec transfer_stmt s l =
      match s with
	| Assert(Lval _)  (* FIXME: Do we want to say v is true? *)
	| Assert _ | Jmp _ | CJmp _ | Label _ | Comment _
	| Return _ | Halt _ | Call(None, _, _) ->
	    l
	| Attr(s,_) ->
	    transfer_stmt s l
	| Call(Some v, _, _) ->
	    VM.remove v l (* set v to top again *)
	| Move(v, e) ->
	    try
	      let top v = SI.top (bits_of_width(typeof_var v)) in
	      let find v = VM.find v l in
	      let do_find v =  try find v with Not_found -> top v  in
	      let val2si = function
		| Int(i,t) -> SI.of_vine_int i t
		| Str _ -> raise(Unimplemented "SI can't represent strings")
		| Name _ -> raise(Unimplemented "No SI for labels (should be a constant)")
		| Lval v -> do_find v
	      in
		try
...
*)
		  ...
		  in
		    if try VM.find v l <> new_si with Not_found -> true then (
		      dprintf "adding %s = %s" (var_to_string v) (SI.to_string new_si);
		      VM.add v new_si l )
		    else l
    ...
    let transfer_function =
      Vine_dataflow.fwd_transfer_stmt_to_block transfer_stmt

  end
  
  module DF = Dataflow.Make(DFP)
...
struct
  ...
  let global = Asmir.x86_mem (* value doesn't really matter, so long as it's unique *)

  let top = []

  let pp_address p (r, si) =
    if r == global then p "$" else Vine.pp_var p r;
    p " |=> ";
    p (SI.to_string si)
...
struct
  ...
  struct
    ...
    let s0 = Vine_cfg.BB_Entry
    let init =
      let sp = List.find (fun (_,n,_) -> n = "R_ESP") Asmir.x86_regs in
	...
	| Move(v, e) ->
		...
		| Str _ -> raise(Unimplemented "VS can't represent strings")
		| Name _ -> raise(Unimplemented "No VS for labels (should be a constant)")
		| Lval v -> do_find v
	      in
		try
...
module AllocEnv = struct
  ...
  module M1 = Map.Make(struct type t = VS.region let compare = Vine.Var.compare end)
...
struct
  ...
  struct
    ...
    *)
    ...
    let init =
      init_vars [List.find (fun (_,n,_) -> n = "R_ESP") Asmir.x86_regs]
