...
(** Debugging module.
    ...
    $VINE_LOGFILE. The default is to write to standard out.
    - [VINE_DEBUG_MODULES]
    ...
    disabled. The format is like that of [VINE_DEBUG_MODULES].
    Default behavior is on (unlike debugging).

    - [VINE_DEBUG_TIMESTAMPS]
...
let (debug_string,debug_endline) =
  try
    let filename = Sys.getenv "VINE_LOGFILE" in
...
(** [has_debug s] returns true when debugging is enabled for s.
    See documentation on [VINE_DEBUG_MODULES] at the top.
*)
let has_debug =
  get_env_options "VINE_DEBUG_MODULES"

(** [has_warn s] returns true when warnings are enabled for s.
    See documention on [VINE_WARN_MODULES] at the top *)
let has_warn = 
  get_env_options "VINE_WARN_MODULES" true
...
let ptime =
  try
    match Sys.getenv "VINE_DEBUG_TIMESTAMPS" with
	"" | "unix" | "Unix" -> ptime_unix
      ...
      | s ->
	  prerr_endline("Warning: Unknown VINE_DEBUG_TIMESTAMPS value ("^s^")");
	  ptime_unix
  with Not_found ->
    ptime_none
