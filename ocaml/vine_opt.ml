 ...
 *)
   ...
   (* Int64.div rounds towards zero. What do we want? *)
	    ...
	    | _ -> e (* ignore other binops on constants for now*)
	 )
    (*  some identities *)
	...
	Cast(ct1, w2, e1) (* no redundact double casts *)
    | Cast(CAST_LOW, REG_1, Cast(CAST_SIGNED, _, e1)) ->
	Cast(CAST_LOW, REG_1, e1)
    | Cast(CAST_LOW, REG_1, Cast(CAST_UNSIGNED, _, e1)) ->
	Cast(CAST_LOW, REG_1, e1)
