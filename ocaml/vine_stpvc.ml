...
let rec vine_to_stp vc ctx e =
  let rec tr e =
    match e with
	UnOp(NOT, e1) -> e_bvnot vc (tr e1)
      | UnOp(NEG, e1) -> e_bvneg vc (tr e1)
      | BinOp(bop,e1,e2) ->
	  let t = Vine_typecheck.infer_type None e in
	  let w = bits_of_width t in
	    (* w2 is only needed for the shift hack *)
	  let w2 = bits_of_width (Vine_typecheck.infer_type None e2) in
	  ...
	  in
	    constr (tr e1) (tr e2)
      | Constant(Int(t, v)) -> e_bv_of_int64 vc (bits_of_width t) v
      | Constant _ -> 
	  ...
	  let e1 = tr e1 in
	    (match ct with
		 CAST_SIGNED    -> e_bvsextend vc w e1
	       | CAST_UNSIGNED  ->
		   if w = w1 then e1 else
		     e_bvconcat vc (e_bv_of_int vc (w-w1) 0) e1
	       | CAST_LOW       -> e_bvextract vc e1 (w-1) 0
	       | CAST_HIGH      -> e_bvextract vc e1 (w1-1) (w1-w)
	    )
      | Let(Temp _, _, _) -> 
	  tr_let [] e
	  ...
	  let a' = Cast(CAST_UNSIGNED,REG_64, a) in
	    e_read vc (VH.find ctx memname) (tr a')
  ...
  and tr_let to_remove e =
	...
	Let(Temp n, e1, e2) ->
      ...
      | Let(Mem(memname,a,_), e1, e2) when is_not_mem memname->
          ...
          let a' = Cast(CAST_UNSIGNED,REG_64, a) in
          let newmem = e_write vc mem (tr a') (tr e1) in
...
let stp_to_vine ?(strip_nums=false) ?(ctx=Hashtbl.create 17) exp =
  ...
  and stp_to_exp e =
	...
	match get_kind e with
	  ...
	  | Libstp.ARRAY ->
	      failwith "vine_stpvc: translation from ARRAY unsupported"
	  | Libstp.BOOLVEC ->
