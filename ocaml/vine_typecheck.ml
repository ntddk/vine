...
let gamma_create () : gamma = Some(VS.empty) 

let gamma_extend gamma (v:var) (t:typ) = 
    match gamma with
      None -> None  (* let g = get_empty_varhash () in 
	VS.add g v t;
	Some(g) *) (* Some(VS.add VS.empty v t)  *)
    | Some(g) -> (* VS.add g v t; Some(g) *)  Some(VS.add g v t) 
;;
...
and typecheck_exp names gamma (e:exp)  = 
  match e with
	...
	if Symbols.mem names l then
	  (names,Vine.addr_t)
	else (Symbols.add names l None, Vine.addr_t)
	) else 
	  (names,Vine.addr_t)
    | Let(lv, e1, e2) ->  
...
let rec typecheck_stmt scope omega names labels sigma gamma s  = 
  ...
  let check_labels_and_names l sigma labels names =  (
    if !typecheck_jmp_targets then (
    try
      let nlst = Symbols.keys names in 
      let l' = List.find
	(fun n ->
	   not (Symbols.mem labels n) &&
	     not (Symbols.mem sigma n)
	) nlst in
  ...
  match s with
    ...
    | Label(l) -> 
	if (!typecheck_jmp_targets) then (
	  if Symbols.mem labels l then
	    (names, labels, sigma, gamma)
	  else 
	    (names,Symbols.add labels l None, sigma,gamma)
	) else
	  (names, labels, sigma, gamma)
    | ExpStmt(e) -> 
	...
	let sigma = Symbols.add sigma l tfun in 
	let gamma' = List.fold_left 
		...
		None -> ()
		  ...
		  let (names,labels, sigma', _) = 
		    typecheck_stmt scope topt 
		      Symbols.empty Symbols.empty sigma gamma' s'
		  in
		    if sigma <> sigma' then
...
let typecheck (dl,sl) = 
  ...
  in
	...
	(Symbols.empty, Symbols.empty ,Symbols.empty, gamma) sl)
      



let infer_type gamma e = 
  let (_, t) = typecheck_exp Symbols.empty gamma e in 
    t
;;
