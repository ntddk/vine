(* File generated from libasmir.idl *)

type exp
and stmt
and asm_function_t
and asm_program_t
and vine_block_t
and vine_blocks_t
and dyn_function_t
and dyn_functions_t
and memory_cell_data_t
and memory_data_t
and instmap_t
and address_t = int64
and _raw_inst_t = {
  address: address_t;
  bytes: char array;
}
and raw_inst_t = _raw_inst_t
and bfd_architecture =
  | Bfd_arch_unknown
  | Bfd_arch_obscure
  | Bfd_arch_m68k
  | Bfd_arch_vax
  | Bfd_arch_i960
  | Bfd_arch_or32
  | Bfd_arch_sparc
  | Bfd_arch_spu
  | Bfd_arch_mips
  | Bfd_arch_i386
  | Bfd_arch_we32k
  | Bfd_arch_tahoe
  | Bfd_arch_i860
  | Bfd_arch_i370
  | Bfd_arch_romp
  | Bfd_arch_convex
  | Bfd_arch_m88k
  | Bfd_arch_m98k
  | Bfd_arch_pyramid
  | Bfd_arch_h8300
  | Bfd_arch_pdp11
  | Bfd_arch_powerpc
  | Bfd_arch_rs6000
  | Bfd_arch_hppa
  | Bfd_arch_d10v
  | Bfd_arch_d30v
  | Bfd_arch_dlx
  | Bfd_arch_m68hc11
  | Bfd_arch_m68hc12
  | Bfd_arch_z8k
  | Bfd_arch_h8500
  | Bfd_arch_sh
  | Bfd_arch_alpha
  | Bfd_arch_arm
  | Bfd_arch_ns32k
  | Bfd_arch_w65
  | Bfd_arch_tic30
  | Bfd_arch_tic4x
  | Bfd_arch_tic54x
  | Bfd_arch_tic80
  | Bfd_arch_v850
  | Bfd_arch_arc
  | Bfd_arch_m32c
  | Bfd_arch_m32r
  | Bfd_arch_mn10200
  | Bfd_arch_mn10300
  | Bfd_arch_fr30
  | Bfd_arch_frv
  | Bfd_arch_mcore
  | Bfd_arch_mep
  | Bfd_arch_ia64
  | Bfd_arch_ip2k
  | Bfd_arch_iq2000
  | Bfd_arch_mt
  | Bfd_arch_pj
  | Bfd_arch_avr
  | Bfd_arch_bfin
  | Bfd_arch_cr16
  | Bfd_arch_cr16c
  | Bfd_arch_crx
  | Bfd_arch_cris
  | Bfd_arch_s390
  | Bfd_arch_score
  | Bfd_arch_openrisc
  | Bfd_arch_mmix
  | Bfd_arch_xstormy16
  | Bfd_arch_msp430
  | Bfd_arch_xc16x
  | Bfd_arch_xtensa
  | Bfd_arch_maxq
  | Bfd_arch_z80
  | Bfd_arch_last
and _vine_symbol_t = {
  name: string;
  addr: address_t;
  is_function: int;
  is_dynamic: int;
}
and vine_symbol_t = _vine_symbol_t
and _cflow_t = {
  ctype: int;
  target: address_t;
}
and cflow_t = _cflow_t
and vine_symbols_t
and exp_type_t =
  | BINOP
  | UNOP
  | CONSTANT
  | MEM
  | TEMP
  | PHI
  | CAST
  | NAME
  | UNKNOWN
  | LET
  | EXTENSION
and reg_t =
  | REG_1
  | REG_8
  | REG_16
  | REG_32
  | REG_64
and binop_type_t =
  | PLUS
  | MINUS
  | TIMES
  | DIVIDE
  | MOD
  | LSHIFT
  | RSHIFT
  | ARSHIFT
  | LROTATE
  | RROTATE
  | LOGICAND
  | LOGICOR
  | BITAND
  | BITOR
  | XOR
  | EQ
  | NEQ
  | GT
  | LT
  | GE
  | LE
  | SDIVIDE
  | SMOD
and unop_type_t =
  | NEG
  | NOT
and const_val_t = int64
and cast_t =
  | CAST_UNSIGNED
  | CAST_SIGNED
  | CAST_HIGH
  | CAST_LOW
  | CAST_FLOAT
  | CAST_INTEGER
  | CAST_RFLOAT
  | CAST_RINTEGER
and stmt_type_t =
  | JMP
  | CJMP
  | SPECIAL
  | MOVE
  | COMMENT
  | LABEL
  | EXPSTMT
  | VARDECL
  | CALL
  | RETURN
  | FUNCTION
  | ASSERT

external exp_type : exp -> exp_type_t
	= "camlidl_libasmir_exp_type"

external binop_type : exp -> binop_type_t
	= "camlidl_libasmir_binop_type"

external binop_lhs : exp -> exp
	= "camlidl_libasmir_binop_lhs"

external binop_rhs : exp -> exp
	= "camlidl_libasmir_binop_rhs"

external unop_type : exp -> unop_type_t
	= "camlidl_libasmir_unop_type"

external unop_subexp : exp -> exp
	= "camlidl_libasmir_unop_subexp"

external mem_addr : exp -> exp
	= "camlidl_libasmir_mem_addr"

external mem_regtype : exp -> reg_t
	= "camlidl_libasmir_mem_regtype"

external constant_val : exp -> const_val_t
	= "camlidl_libasmir_constant_val"

external constant_regtype : exp -> reg_t
	= "camlidl_libasmir_constant_regtype"

external phi_phiname : exp -> string
	= "camlidl_libasmir_phi_phiname"

external phi_numnodes : exp -> int
	= "camlidl_libasmir_phi_numnodes"

external phi_nodeat : exp -> int -> exp
	= "camlidl_libasmir_phi_nodeat"

external temp_regtype : exp -> reg_t
	= "camlidl_libasmir_temp_regtype"

external temp_name : exp -> string
	= "camlidl_libasmir_temp_name"

external unknown_str : exp -> string
	= "camlidl_libasmir_unknown_str"

external cast_width : exp -> reg_t
	= "camlidl_libasmir_cast_width"

external cast_casttype : exp -> cast_t
	= "camlidl_libasmir_cast_casttype"

external cast_subexp : exp -> exp
	= "camlidl_libasmir_cast_subexp"

external name_string : exp -> string
	= "camlidl_libasmir_name_string"

external let_var : exp -> exp
	= "camlidl_libasmir_let_var"

external let_exp : exp -> exp
	= "camlidl_libasmir_let_exp"

external let_in : exp -> exp
	= "camlidl_libasmir_let_in"

external stmt_type : stmt -> stmt_type_t
	= "camlidl_libasmir_stmt_type"

external move_lhs : stmt -> exp
	= "camlidl_libasmir_move_lhs"

external move_rhs : stmt -> exp
	= "camlidl_libasmir_move_rhs"

external label_string : stmt -> string
	= "camlidl_libasmir_label_string"

external special_string : stmt -> string
	= "camlidl_libasmir_special_string"

external comment_string : stmt -> string
	= "camlidl_libasmir_comment_string"

external jmp_target : stmt -> exp
	= "camlidl_libasmir_jmp_target"

external cjmp_cond : stmt -> exp
	= "camlidl_libasmir_cjmp_cond"

external cjmp_ttarget : stmt -> exp
	= "camlidl_libasmir_cjmp_ttarget"

external cjmp_ftarget : stmt -> exp
	= "camlidl_libasmir_cjmp_ftarget"

external expstmt_exp : stmt -> exp
	= "camlidl_libasmir_expstmt_exp"

external vardecl_name : stmt -> string
	= "camlidl_libasmir_vardecl_name"

external vardecl_type : stmt -> reg_t
	= "camlidl_libasmir_vardecl_type"

external call_has_lval : stmt -> bool
	= "camlidl_libasmir_call_has_lval"

external call_lval_opt : stmt -> exp
	= "camlidl_libasmir_call_lval_opt"

external call_fnname : stmt -> exp
	= "camlidl_libasmir_call_fnname"

external call_params : stmt -> exp array
	= "camlidl_libasmir_call_params"

external ret_has_exp : stmt -> bool
	= "camlidl_libasmir_ret_has_exp"

external ret_exp : stmt -> exp
	= "camlidl_libasmir_ret_exp"

external func_name : stmt -> string
	= "camlidl_libasmir_func_name"

external func_has_rv : stmt -> bool
	= "camlidl_libasmir_func_has_rv"

external func_rt : stmt -> reg_t
	= "camlidl_libasmir_func_rt"

external func_params : stmt -> stmt array
	= "camlidl_libasmir_func_params"

external func_is_external : stmt -> bool
	= "camlidl_libasmir_func_is_external"

external func_body : stmt -> stmt array
	= "camlidl_libasmir_func_body"

external assert_cond : stmt -> exp
	= "camlidl_libasmir_assert_cond"

external gen_eflags_helpers_c : unit -> stmt array
	= "camlidl_libasmir_gen_eflags_helpers_c"

external disassemble_program : string -> asm_program_t
	= "camlidl_libasmir_disassemble_program"

external instmap_to_asm_program : instmap_t -> asm_program_t
	= "camlidl_libasmir_instmap_to_asm_program"

external filename_to_instmap : string -> instmap_t
	= "camlidl_libasmir_filename_to_instmap"

external instmap_translate_address_range : instmap_t -> address_t -> address_t -> vine_blocks_t
	= "camlidl_libasmir_instmap_translate_address_range"

external instmap_translate_address : instmap_t -> address_t -> vine_block_t
	= "camlidl_libasmir_instmap_translate_address"

external instmap_has_address : instmap_t -> address_t -> int
	= "camlidl_libasmir_instmap_has_address"

external get_last_segment_address : string -> address_t -> address_t
	= "camlidl_libasmir_get_last_segment_address"

external asmir_asmprogram_to_vine : asm_program_t -> vine_blocks_t
	= "camlidl_libasmir_asmir_asmprogram_to_vine"

external asmir_vine_blocks_size : vine_blocks_t -> int
	= "camlidl_libasmir_asmir_vine_blocks_size"

external asmir_vine_blocks_get : vine_blocks_t -> int -> vine_block_t
	= "camlidl_libasmir_asmir_vine_blocks_get"

external destroy_vine_block : vine_block_t -> unit
	= "camlidl_libasmir_destroy_vine_block"

external destroy_vine_blocks : vine_blocks_t -> unit
	= "camlidl_libasmir_destroy_vine_blocks"

external asmir_vine_block_size : vine_block_t -> int
	= "camlidl_libasmir_asmir_vine_block_size"

external asmir_vine_block_address : vine_block_t -> address_t
	= "camlidl_libasmir_asmir_vine_block_address"

external asmir_vine_block_get : vine_block_t -> int -> stmt
	= "camlidl_libasmir_asmir_vine_block_get"

external free_asm_function : asm_function_t -> unit
	= "camlidl_libasmir_free_asm_function"

external free_asm_program : asm_program_t -> unit
	= "camlidl_libasmir_free_asm_program"

external asm_addr_to_ir : asm_program_t -> address_t -> vine_block_t
	= "camlidl_libasmir_asm_addr_to_ir"

external string_blockinsn : asm_program_t -> vine_block_t -> string
	= "camlidl_libasmir_string_blockinsn"

external get_synthetic_symbols : string -> dyn_functions_t
	= "camlidl_libasmir_get_synthetic_symbols"

external dyn_functions_size : dyn_functions_t -> int
	= "camlidl_libasmir_dyn_functions_size"

external dyn_functions_get : dyn_functions_t -> int -> dyn_function_t
	= "camlidl_libasmir_dyn_functions_get"

external dyn_functions_name : dyn_function_t -> string
	= "camlidl_libasmir_dyn_functions_name"

external dyn_functions_addr : dyn_function_t -> address_t
	= "camlidl_libasmir_dyn_functions_addr"

external destroy_dyn_functions : dyn_functions_t -> unit
	= "camlidl_libasmir_destroy_dyn_functions"

external get_symbols_of_file : string -> vine_symbols_t
	= "camlidl_libasmir_get_symbols_of_file"

external symbols_size : vine_symbols_t -> int
	= "camlidl_libasmir_symbols_size"

external symbols_get : vine_symbols_t -> int -> vine_symbol_t option
	= "camlidl_libasmir_symbols_get"

external destroy_symbols : vine_symbols_t -> unit
	= "camlidl_libasmir_destroy_symbols"

external memory_cell_data_address : memory_cell_data_t -> address_t
	= "camlidl_libasmir_memory_cell_data_address"

external memory_cell_data_value : memory_cell_data_t -> int
	= "camlidl_libasmir_memory_cell_data_value"

external get_rodata : asm_program_t -> memory_data_t
	= "camlidl_libasmir_get_rodata"

external get_bssdata : asm_program_t -> memory_data_t
	= "camlidl_libasmir_get_bssdata"

external memory_data_size : memory_data_t -> int
	= "camlidl_libasmir_memory_data_size"

external memory_data_get : memory_data_t -> int -> memory_cell_data_t
	= "camlidl_libasmir_memory_data_get"

external destroy_memory_data : memory_data_t -> unit
	= "camlidl_libasmir_destroy_memory_data"

external set_use_eflags_thunks : bool -> unit
	= "camlidl_libasmir_set_use_eflags_thunks"

external get_use_eflags_thunks : unit -> bool
	= "camlidl_libasmir_get_use_eflags_thunks"

external set_call_return_translation : bool -> unit
	= "camlidl_libasmir_set_call_return_translation"

external asmprogram_arch : asm_program_t -> bfd_architecture
	= "camlidl_libasmir_asmprogram_arch"

external print_disasm_rawbytes : bfd_architecture -> address_t -> char array -> unit
	= "camlidl_libasmir_print_disasm_rawbytes"

external byte_insn_to_asmp : bfd_architecture -> address_t -> char array -> asm_program_t
	= "camlidl_libasmir_byte_insn_to_asmp"

