...
type kind = Libstp.exprkind_t (** The STP kind of an STP expression *)
type type_t = Libstp.type_t (** The low-level STP type of an STP expression *)
...
val e_sbvdiv : vc -> int -> exp -> exp -> exp
val e_sbvmod : vc -> int -> exp -> exp -> exp
...
val get_kind : exp -> kind
  (** @return kind of expression *)

val get_degree : exp -> int
  (** @return number of children expression *)

val get_child : exp -> int -> exp
  (** [get_child e ith] return the [ith] child of [e] *)
  
val get_bvlength : exp -> int
  (** @return the bit width of an expression. The expression must be a 
      bitvector
  *)

val get_type : exp -> type_t
  (** @return the low-level type of expression *)

val get_vwidth : exp -> int
  (** @return the bit width of an expression. If an expression is an array,
      this return the value width *)

val get_iwidth : exp -> int
  (** @return the index width of an expression. The expression must be
      an array *)

val get_name : exp -> string
  (** @return the string representation of the given expression *)

val get_id : exp -> int
  (** @ return the ID of the given STP expression *)

val get_true_counterexample : vc -> ( exp * exp ) list
  (** @return the pairs of counter example as in the command-line STP *)
